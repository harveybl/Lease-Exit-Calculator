...
must_haves:
  truths:
    ...
    - "UI can render collapsible panels, visual rank badges, and styled content cards for the comparison layout"
  artifacts:
    - path: "src/lib/utils/format-currency.ts"
      provides: "Currency formatting utility wrapping Intl.NumberFormat, plus ScenarioType display name mapper"
      exports: ["formatCurrency", "formatOptionName"]
...
<task type="auto">
  ...
  <verify>
    ...
    - `grep -q 'export.*formatOptionName' src/lib/utils/format-currency.ts` confirms formatOptionName is exported
    - `grep -c "=>" src/lib/utils/format-currency.ts` returns at least 5 (one mapping per ScenarioType: return, buyout, sell-privately, early-termination, extension)
  </verify>
  <done>
